/* Configured KIDS classes: per-page identity hint only; no storage or parent-token access. */
(function(root) {
  'use strict';
  ...
  function read(search, options) {
    ...
    if (names.length !== 1 || classes.length !== 1 || !name || name.length > 60 ||
        /[\u0000-\u001f\u007f]/.test(String(names[0] || '')) || !allowedClasses || !allowedClasses.has(className)) {
  ...
  function install(options, targetWindow, explicitIdentity) {
    ...
    const hint = explicitIdentity === undefined ? read(undefined, config) : validatedIdentity(explicitIdentity, config);
