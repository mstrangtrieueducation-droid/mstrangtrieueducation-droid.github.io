...
(function () {
  ...
  function nativeFormTarget(value) {
    try {
      const url = new URL(value);
      return url.origin === 'https://docs.google.com' && !url.username && !url.password &&
        /^\/forms\/(?:u\/\d+\/)?d\/(?:e\/)?[A-Za-z0-9_-]+\/viewform\/?$/.test(url.pathname) ? url : null;
    } catch (_) { return null; }
  }
  function lessonTarget(value) {
    try {
      ...
      if (!first || first === 'kids-learning' || first === 'student-learning' || denied.has(first) || parts.some(part =>
  ...
  const nativeForm = values.length === 1 ? nativeFormTarget(values[0]) : null;
  const target = values.length === 1 ? lessonTarget(values[0]) || nativeForm : null;
  const hint = receiver.read(location.search, config);
  ...
  if (nativeForm) {
    const match = nativeForm.pathname.match(/\/d\/(e\/)?([^/]+)\/viewform\/?$/);
    const mapping = config.forms.find(item => match[1] ? item.formId === match[2] : (item.aliases || []).includes(match[2]));
    original.textContent = 'Mở biểu mẫu';
    if (!mapping) {
      original.href = nativeForm.href;
      message.textContent = 'Biểu mẫu này chưa thể tự điền tên và lớp. Bạn có thể mở biểu mẫu và kiểm tra thông tin trước khi nộp.';
      help.hidden = false; retry.hidden = true;
      return;
    }
    const destination = receiver.formUrl(nativeForm.href, mapping, hint);
    original.href = destination;
    message.textContent = 'Đang mở biểu mẫu nộp bài của ' + hint.name + '…';
    window.location.replace(destination);
    return;
  }
  // Some existing upload helpers redirect before DOMContentLoaded. Resolve the
  // source-proven family rule first, using the same receiver as lesson buttons.
  const directForm = receiver.resolveFormRedirect(target.href, config, hint);
  if (directForm) {
    original.href = directForm;
    message.textContent = 'Đang mở biểu mẫu nộp bài của ' + hint.name + '…';
    window.location.replace(directForm);
    return;
  }
