...
(function(root) {
  ...
  function formTarget(value, mapping, baseUrl) {
    ...
    if (url.origin !== 'https://docs.google.com' || url.username || url.password) return null;
    ...
    if (match[1]) {
      if (match[2] !== mapping.formId) return null;
    } else {
      // Private/editor Form IDs are aliases only when their published mapping
      // has been verified. Never infer a published ID from another URL shape.
      if (!Array.isArray(mapping.aliases) || !mapping.aliases.includes(match[2]) ||
          !/^[A-Za-z0-9_-]+$/.test(mapping.formId || '')) return null;
      url.pathname = '/forms/d/e/' + mapping.formId + '/viewform';
    }
    return url;
  }
  ...
  function resolveSubmissionRedirect(value, config, hint, baseUrl) {
    const groups = config && config.submissionRedirects;
    if (!Array.isArray(groups) || !hint || hint.state === 'absent') return null;
    try {
      ...
      groups.forEach(group => {
        if (!group || !Array.isArray(group.paths) || !Array.isArray(group.routes) ||
            !group.paths.some(path => path === url.pathname || path === url.pathname + '/')) return;
        const code = String(url.searchParams.get(group.queryKey || 'code') || '').trim().toUpperCase();
        group.routes.forEach(route => {
          if (route && typeof route.code === 'string' && route.code === code) routes.push(route);
        });
      });
      if (routes.length !== 1) return null;
      const route = routes[0];
      const mappings = (config.forms || []).filter(item => formTarget(route.url, item, base.href));
      if (mappings.length !== 1) return null;
      // These source-proven redirects replace the entire destination. They do
      // not carry incoming parameters or manufacture an assignment entry.
      return formUrl(route.url, mappings[0], hint, base.href);
    } catch (_) { return null; }
  }
  function resolveFormRedirect(value, config, hint, baseUrl) {
    const submission = resolveSubmissionRedirect(value, config, hint, baseUrl);
